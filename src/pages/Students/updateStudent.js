...
import { updateStudentData, getStudent } from '../../RequestManagement/studentManagement';
import {
    listLevelWYearEduc,
...
function UpdateStudent() {
    ...
    const fetchStudentData = async () => {
        ...
        if (result.code === 200) {
            const usersData = result.student;
            const image = await usersData.personProfile2.imagePath !== null && usersData.personProfile2.imagePath !== '' ?
                `data:image/jpeg;base64,${Buffer.from(
                    usersData.personProfile2.imagePath).toString("base64")}` : "../../assets/images/avatars/avatar_10.jpg";
            setImage(image);
            setFirstName(usersData.personProfile2.firstName);
            setLastName(usersData.personProfile2.lastName);
            setMail(usersData.personProfile2.mail);
            setPhoneNumber(usersData.personProfile2.phoneNumber);
            setDateOfBirth(usersData.personProfile2.dateOfBirth);
            setSelectedLevel(usersData.studentLevel.levelID ? usersData.studentLevel.levelID.toString() : "");
            if (usersData.studentLevel.levelID && usersData.studentLevel.levelID !== "") {

                const foundElement = await levelsData.find(item => item.ID_ROWID === usersData.studentLevel.levelID);
            ...
            await setSelectedYear(usersData.studentLevel.yearID ? usersData.studentLevel.yearID.toString() : "");
            const urls = await Promise.all(result.files.map(async (file) => {
                const data = `data:${file.type};base64,${Buffer.from(file.data).toString("base64")}`;
    ...
    return (
        <Container>
                ...
                <Card style={{ padding: '20px' }}>
                    <form onSubmit={handleSubmit}>
                        <Grid container spacing={3}>
                                ...
                                <FormControl fullWidth>
                                    ...
                                    >
                                        <MenuItem onClick={handleOpenSetingsLevelEd} value="">
                                            <Typography variant="title" style={{ color: "blue" }} gutterBottom component="div">
                            ...
                            <Grid item xs={12} container justifyContent="center" alignItems="center" >
                                <input type="file" name="docs" id="docs-upload" accept="image/*, application/pdf" onChange={handleImageUpload} style={{ display: 'none' }} />
                                <Button component="label" variant="contained" startIcon={<CloudUploadIcon />}>
            ...
            <LevelSetting isOpen={isLevelSetting} handleCancelClick={handleCancelClick} handelReFetch={fetchLevelsData} />
